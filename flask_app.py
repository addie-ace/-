...
# 修改数据库文件路径为PythonAnywhere可写目录
LICENSE_DB_FILE = "/home/a814003570/mysite/license_database.json"
LOG_FILE = "/home/a814003570/mysite/admin_logs.json"
...
def load_database():
    ...
    if os.path.exists(LICENSE_DB_FILE):
        with open(LICENSE_DB_FILE, 'r', encoding='utf-8') as f:
            return json.load(f)
    return {'licenses': {}, 'admin_logs': []}

def save_database(db):
    """保存序列号数据库"""
    os.makedirs(os.path.dirname(LICENSE_DB_FILE), exist_ok=True)
...
def init_database():
    ...
    if not os.path.exists(LICENSE_DB_FILE):
        ...
        # 保存到文件
        save_database(db)
        log_admin_action(db, 'init_database', {'message': '初始化数据库'})
...
def verify_license():
    ...
    try:
        ...
        try:
            ...
            return jsonify({'success': False, 'message': '序列号格式无效'})
        ...
        # 记录操作日志
        log_admin_action(db, 'verify_license', {
...
def check_license():
    ...
    try:
        data = request.get_json()
        if not data:
            return jsonify({'success': True, 'message': '服务器正常'})
...
def admin_list_licenses():
    ...
    try:
        ...
        # 整理序列号信息
        licenses = []
        for license_id, license_info in db.get('licenses', {}).items():
            licenses.append({
                'license_id': license_id,
                ...
                'last_check': license_info.get('last_check')
            })
...
def admin_unbind_license():
    ...
    try:
        ...
        # 检查序列号是否存在
        if license_id not in db.get('licenses', {}):
            return jsonify({'success': False, 'message': '序列号不存在'})
